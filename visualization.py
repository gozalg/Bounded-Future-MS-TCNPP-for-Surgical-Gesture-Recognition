...
def plot_seq_gestures(all_recogs, all_gt,all_names,task,best):
    if task == "gesture":
        cmap_ = "tab10"
...
def save_all_seq(all_recogs, all_gt,all_names,task):
    if task == "gesture":
        cmap_ = "tab10"
        gest_to_int = {"G0": 0, "G1": 1, "G2": 2, "G3": 3, "G4": 4, "G5": 5}
        vmax_ = 6
    ...
    for split_gt,split_pred,split_name in zip(all_gt,all_recogs,all_names):
        for gt,pred,name in zip(split_gt,split_pred,split_name):
            name = name[:-4]
            gestures_list = [pred, gt]
            fig, axs = plt.subplots(len(gestures_list), 1, figsize=(14,  0.7*len(gestures_list)))
