...
parser.add_argument('--clip_length_past', type=int, default=15, 
                   help="Number of frames to clip from the past (beginning) for causal evaluation")
parser.add_argument('--clip_length_future', type=int, default=0, 
                   help="Number of frames to clip from the future (end) for causal evaluation")
...
# Add delay configuration to experiment name if specified
if args.clip_length_future > 0 or args.clip_length_past > 0:
    experiment_name += f" pst: {args.clip_length_past} ftr: {args.clip_length_future}"
